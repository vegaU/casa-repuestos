...
{
    public function purchase(Request $request, Tenant $tenant, Purchase $purchase, InventoryService $inventory)
    {
        abort_unless($purchase->tenant_id === $tenant->id, 404);
        $reason = $request->validate(['reason' => ['required', 'string', 'max:1000']])['reason'];

        return ['data' => DB::transaction(function () use ($purchase, $reason, $inventory) {
    ...
    public function sale(Request $request, Tenant $tenant, Sale $sale, InventoryService $inventory)
    {
        abort_unless($sale->tenant_id === $tenant->id, 404);
        $reason = $request->validate(['reason' => ['required', 'string', 'max:1000']])['reason'];

        return ['data' => DB::transaction(function () use ($sale, $reason, $inventory) {
